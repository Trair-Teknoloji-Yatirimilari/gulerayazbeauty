    ...
    <div className="min-h-screen bg-background pt-24 pb-16 px-6">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            <Link to="/admin" className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-primary hover:text-primary mb-2">
        ...
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-border/40 bg-card/40 overflow-hidden overflow-x-auto">
              ...
              <tbody>
                ...
                {data?.map((post) => (
                  <tr key={post.id} className="border-t border-border/30 hover:bg-background/40 transition">
                    ...
                    <td className="p-4">
                      <span className={`inline-block px-2.5 py-1 rounded-full text-[11px] uppercase tracking-wider border ${post.status === "published" ? "border-emerald-500/30 text-emerald-600 bg-emerald-500/10" : "border-amber-500/30 text-amber-600 bg-amber-500/10"}`}>
